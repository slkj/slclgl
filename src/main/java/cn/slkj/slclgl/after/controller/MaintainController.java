...
import cn.slkj.slclgl.after.service.MaintainService;
...
public class MaintainController {
	...
	public EPager<Maintain> list(@RequestParam(required = false, defaultValue = "1") Integer page,
		...
		List<Maintain> list = service.search(map);
		return new EPager<Maintain>(total, list);
	}

	@ResponseBody
	@RequestMapping(value = "/one")
	public Maintain queryOne(String id) throws Exception {
		try {
			Maintain p = new Maintain();
			p.setId(id);
	...
	public boolean save(Maintain p) throws Exception {
		try {
	...
	public boolean editSave(Maintain p) throws Exception {
		try {
